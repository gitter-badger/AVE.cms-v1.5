...
function get_pagination($total_pages, $type, $template_label, $navi_box = '')
{
	...
	{
		//Первая
		$nav .= sprintf($link_box, str_replace('{t}', $start_label, str_replace(array('&amp;'.$type.'={s}','&'.$type.'={s}','/'.$type.'-{s}'), '', $template_label)));
		if ($separator_label != '') $nav .= sprintf($separator_box, $separator_label);
	}
	...
	if ($curent_page > 1)
	{
		...
		{
			//Страница 2
			$nav .= sprintf($link_box, str_replace('{t}', $prev_label, str_replace(array('&amp;'.$type.'={s}','&'.$type.'={s}','/'.$type.'-{s}'), '', $template_label)));
		}
		else
		{
			//Предыдущая
	...
	foreach($pages as $val)
	{
		if ($val >= 1 && $val <= $total_pages)
		{
			...
			else
			{
				...
				{
					// Страница номер 1
					$nav .= sprintf($link_box, str_replace('{t}', $val, str_replace(array('&amp;'.$type.'={s}','&'.$type.'={s}','/'.$type.'-{s}'), '', $template_label)));
				}
				else
				{
	...
	{
		//Страница ХХХ из ХХХ
		if ($total_label != '') $nav = sprintf($total_box, sprintf($total_label, $curent_page, $total_pages)) . $nav;
