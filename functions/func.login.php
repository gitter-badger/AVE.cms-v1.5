...
function user_login($login, $password, $attach_ip = 0, $keep_in = 0, $sleep = 0)
{
	...
	$u_ip = ($attach_ip == 1) ? ip2long($_SERVER['REMOTE_ADDR']) : 0;
	...
	if ($keep_in == 1)
	{
		...
		$sql = "DELETE FROM " . PREFIX . "_users_session WHERE `hash`='" . addslashes($auth) . "'";
		$AVE_DB->Query($sql);

		$sql = "INSERT INTO " . PREFIX . "_users_session (`user_id`,`hash`,`ip`,`agent`,`last_activ`)values('" . $row->Id . "','" . addslashes($auth) . "','" . $u_ip . "','" . addslashes($_SERVER['HTTP_USER_AGENT']) . "','" . time() . "')";
		$AVE_DB->Query($sql);
...
function auth_cookie()
{
	...
	$sql = "
		...
		AND `agent` = '".addslashes($_SERVER['HTTP_USER_AGENT'])."'
	";
	...
	$row = $AVE_DB->Query("
		...
		WHERE usr.Id = '" . $user_id . "' AND usrs.hash = '".$_COOKIE['auth']."'
		LIMIT 1
	")->FetchRow();
	...
	if (USER_IP) {
		...
		$sql = "DELETE FROM ".PREFIX."_users_session WHERE `hash`='".addslashes($_COOKIE['auth'])."'";
		$AVE_DB->Query($sql);
		@setcookie('auth', '', 0, ABS_PATH, $cookie_domain);
		return false;
	}
