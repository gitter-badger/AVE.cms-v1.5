...
//			if($row->document_lang_group > 0){
				$sql1=$AVE_DB->Query("SELECT
					...
					WHERE document_lang_group=".$row->document_lang_group." OR Id=".$row->document_lang_group);
...
//			}
		...
		if ($num > $limit)
		{
			$page_nav = get_pagination($pages, 'page', ' <a href="' . $link . $navi_sort . '&page={s}'.(empty($_REQUEST['rubric_id']) ? '' : '&rubric_id='.$_REQUEST['rubric_id']).'&cp=' . SESSION . '">{t}</a>');
	...
	*/
	...
	function documentSave($rubric_id, $document_id, $data, $update_non_exists_fields = false, $rubric_code = true){
		...
		$_rubric = $AVE_DB->Query("SELECT rubric_alias, rubric_alias_history, rubric_code_start, rubric_code_end FROM " . PREFIX . "_rubrics WHERE Id = '" . $rubric_id . "'")->FetchRow();
		...
		if($oper == 'UPDATE'){
				...
				else
				{
					...
					$data["document_published"]		= ($document_id == 1 || $document_id == PAGE_NOT_FOUND_ID) ? '0' : $this->_documentStart($data['document_published']);
					$data["document_expire"] 		= ($document_id == 1 || $document_id == PAGE_NOT_FOUND_ID) ? '0' : $this->_documentEnd($data['document_expire']);
					$data["document_changed"] 		= time();

					$data['document_alias_history'] = (empty($data['document_alias_history'])) ? '0' : $data['document_alias_history'];
						...
						$breadcrum_title = (isset($data['doc_breadcrum_title']) && $data['doc_breadcrum_title'] != '')
							? $data['doc_breadcrum_title']
					...
					// Сохраняем системное сообщение в журнал
					reportLog(($oper=='INSERT' ? 'Добавил' : 'Отредактировал').' документ (' . get_document($document_id, 'document_title') . ')');
					...
					foreach ($fields as $k => $v)
					{
						...
						$operator = ($oper=='UPDATE' ? "UPDATE " . PREFIX . "_document_fields" : "INSERT INTO " . PREFIX . "_document_fields");
						$fval = (is_array($fld_val) ? serialize($fld_val) : ($fld_val));
						...
						$sql="
							...
							SET
								...
								document_id        = '" . $document_id . "',
								field_value        = '" . mb_substr($fval,0,500) . "',
								field_number_value = '" . (($field_rubric->rubric_field_numeric) ? preg_replace('/[^\d.]/', '', $fld_val) : 0) . "',
								document_in_search = '" . $search . "'
							$where
						";
						...
						if(mb_strlen($fval) > 500)
						{
							$ins1=$AVE_DB->Query('SELECT Id FROM '.PREFIX . '_document_fields_text WHERE document_id='.$document_id.' AND rubric_field_id='.$fld_id)->GetCell();
							$operator = ($ins1>0 ? "UPDATE " . PREFIX . "_document_fields_text" : "INSERT INTO " . PREFIX . "_document_fields_text");
							$where = ($ins1>0 ? 'WHERE document_id='.$document_id.' AND rubric_field_id='.$fld_id : '');
							$sql="
								$operator
								SET
									rubric_field_id    = '" . $fld_id . "',
									document_id        = '" . $document_id . "',
									field_value        = '" . mb_substr($fval,500) . "'
								$where
							";
						$AVE_DB->Query($sql);
						}
						...
						{
							$AVE_DB->Query("DELETE FROM ". PREFIX . "_document_fields_text ".'WHERE document_id='.$document_id.' AND rubric_field_id='.$fld_id);
						}
					}
		...
		switch ($_REQUEST['sub'])
		{
			...
			case '':
				...
				$sql = $AVE_DB->Query("
					...
					FROM ".PREFIX."_documents
					WHERE
					". ($document->document_lang_group > 0 ? "document_lang_group=".$document->document_lang_group." OR Id=".$document->document_lang_group." OR ": "")."document_lang_group=".$document_id." OR Id=".$document_id
				);
				...
				if ($show)
				{
					...
					$sql = $AVE_DB->Query("
						SELECT *
						FROM " . PREFIX . "_rubric_fields
						WHERE rubric_id = '" . $document->rubric_id . "'
						ORDER BY rubric_field_position ASC
					");
					...
					if ( (isset($_SESSION[$document->rubric_id . '_delrev'])  && $_SESSION[$document->rubric_id . '_delrev'] == 1)
						|| (isset($_SESSION[$document->rubric_id . '_alles']) && $_SESSION[$document->rubric_id . '_alles']  == 1)
						|| (defined('UGROUP') && UGROUP == 1) ){ $document->canDelRev = 1; }
	...
	function documentDelete($document_id)
	{
		...
		if ($document_id != 1 && $document_id != PAGE_NOT_FOUND_ID)
		{
			...
			// Сохраняем системное сообщение в журнал
			reportLog('Удалил документ (' . $document_id . ')');
		}
