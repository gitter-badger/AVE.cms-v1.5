	...
	{foreach name=cond from=$conditions item=condition}
		<tr class="cond_tr" data-id="cond_{$condition->Id}">
			...
			<td><input name="condition_status[{$condition->Id}]" type="checkbox" id="condition_status{$condition->Id}" value="1" {if $condition->condition_status ==1}checked{/if} class="toprightDir float" /></td>
				...
				{foreach from=$fields_list item=field_group}
					...
					{foreach  from=$field_group.fields item=field}
						...
						<div id="req_cond_{$condition->Id}">
							...
							<input type="hidden" name="condition_field_id[{$condition->Id}]" value="{$field.Id|escape}" />
						</div>
						{/if}
					{/foreach}
			...
			<td>
				<select style="max-height: 100px;" name="condition_compare[{$condition->Id}]" id="operator_{$condition->Id}">
			...
			<td>
				<select style="width:60px" name="condition_join[{$condition->Id}]">
			...
			<td><div class="pr12"><input name="condition_value[{$condition->Id}]" type="text" id="Wert_{$condition->Id}" value="{$condition->condition_value|escape}" class="mousetrap" /> </div></td>
