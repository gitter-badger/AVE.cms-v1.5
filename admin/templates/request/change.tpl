{if $smarty.request.sub == ''}
	<div id="req_cond_{$cond_id}" stye="text-align: left;">
			<select class="mousetrap" name="condition_field_id[{$cond_id}]" style="width: 200px;" id="form_cond_{$cond_id}">
	...
	{foreach from=$fields_list item=field_group}
		...
		{foreach  from=$field_group.fields item=field}
			...
			<div id="req_cond_{$cond_id}">
				...
				<input type="hidden" name="condition_field_id[{$cond_id}]" value="{$field.Id|escape}" />
			</div>
			{/if}
		{/foreach}
